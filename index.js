...
async function run() {
  try {
    ...
    //* admin er ta , manager ta ki , email ta ki vabe korbo
    app.get("/stats/:email", async (req, res) => {
      try {
        ...
        const managerCount = await userCollection.countDocuments({
          role: "manager",
        });
        const memberCount = await userCollection.countDocuments({
          role: "member",
        });
        const club = await clubsCollection.countDocuments({
          role: userEmail,
        });
    ...
    app.get("/admin/stats", async (req, res) => {
      try {
        // * email ta kothai add korbo backend e  ?? kothai ?
        const [
          clubsCount,
          eventsCount,
          eventsRegCount,
          ...
          clubsCollection.countDocuments({}), //* ok*** thanks you
        ...
        //!
        //* Top 5 clubs by membership count
        ...
        // * Memberships over time (last 6 months) – great for line chart
        const membershipsOverTime = await membershipsCollection
          .aggregate([
            {
              $match: {
                status: "active",
        ...
        // res.json({ membershipsByClub });
        // res.json(counts, { membershipsByClub });
        res.json(counts);
      } catch (err) {
    ...
    app.get("/member/stats/:email", async (req, res) => {
      ...
      try {
        const [
          ...
          clubsCollection.countDocuments({ userEmail: email }),
          eventsCollection.countDocuments({ userEmail: email }),
          ...
          userCollection.countDocuments({ userEmail: email }),
        ]);
    ...
    app.get("/manager/stats/:email", async (req, res) => {
      ...
      try {
        const [
          ...
          clubsCollection.countDocuments({ managerEmail: email }),
          eventsCollection.countDocuments({ managerEmail: email }),
          eventsRegistrationCollection.countDocuments({}),
          membershipsCollection.countDocuments({ managerEmail: email }),
          paymentsCollection.countDocuments({}),
          userCollection.countDocuments({}),
        ]);
    ...
    app.delete("/event/:id", verifyJWT, async (req, res) => {
      ...
      const id = { _id: new ObjectId(eventId) };
      const result = await eventsCollection.deleteOne(id);
      console.log(result);
      res.send(result);
    });
    app.delete("/clubMember/:id", verifyJWT, async (req, res) => {
      const Id = req.params.id;
      const id = { _id: new ObjectId(Id) };
      const result = await membershipsCollection.deleteOne(id);
      console.log(result);
      res.send(result);
    });
    //!=============end===========!//
